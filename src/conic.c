...
/*   Updated: 2019/08/26 14:00:07 by nrivoire    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		projection_conic(t_env *v)
{
	...
	while (++inc < v->max)
	{
		...
		v->cur[inc].x = ((v->map[inc].y + (v->map[inc].z * -v->elev)) \
				+ v->horizon) * sin(deg * v->var);
		v->cur[inc].y = ((v->map[inc].y + (v->map[inc].z * -v->elev)) \
				+ v->horizon) * cos(deg * v->var);
	}
}
